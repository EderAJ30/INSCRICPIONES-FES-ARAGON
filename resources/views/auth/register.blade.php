...
<head>
  ...
  <title>Crear Cuenta - FES Aragón</title>
...
<body class="bg-gradient-to-br from-[#00152D] via-[#002B5C] to-[#001A38] min-h-screen font-sans text-gray-200 antialiased selection:bg-[#C5911F] selection:text-white flex items-center justify-center p-4">
  ...
  <div class="w-full max-w-md bg-[#00152D]/40 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-[0_8px_32px_rgba(0,0,0,0.3)] my-6">
    ...
    <form action="{{ route('register') }}" method="POST" class="space-y-5">
      ...
      <div>
        <label for="numero_cuenta" class="block text-sm font-medium text-gray-300 mb-1">Número de Cuenta</label>
        <input type="text" name="numero_cuenta" id="numero_cuenta" value="{{ old('numero_cuenta') }}" required pattern="\d{9}" title="Debe contener 9 dígitos"
      ...
      <div>
        ...
        <input type="email" name="correo_electronico" id="correo_electronico" value="{{ old('correo_electronico') }}" required autocomplete="email"
          class="w-full bg-[#001A38]/50 border border-white/10 rounded-lg px-4 py-2.5 text-gray-200 focus:outline-none focus:ring-2 focus:ring-[#C5911F] focus:border-transparent transition-all">
    ...
    <p class="mt-6 text-center text-sm text-gray-400">
      ¿Ya tienes cuenta? <a href="{{ route('login') }}" class="text-[#C5911F] hover:text-white transition-colors">Inicia sesión</a>
    </p>
  </div>
